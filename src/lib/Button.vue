...
interface ButtonProps {  // ? 表示可选的 接口里的属性不全都是必需的
...
export default {
  props: {
    theme: {
      type: String as PropType<ButtonThemeType>,  // 类型断言as；如果不加，那么它只是一个单纯的String类型 但使用PropType<ButtonThemeType>，theme就变成了一个类型为ButtonThemeType的String类型
      default: "button",
    ...
    level: {
      ...
      default: "normal",
      validator: (value: string) => {
        return ["normal", "main", "danger", "warning"].includes(value);
      }
    },
    disabled: {
      type: Boolean,
